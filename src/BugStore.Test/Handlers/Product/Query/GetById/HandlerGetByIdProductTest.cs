...
namespace BugStore.Test.Handlers.Product.Query.GetById;
...
public class HandlerGetByIdProductTest
{
    ...
    public async Task GetByIdProductOk_Return_Product()
    {
        ...
        var result = await _handler.Handle(new Requests.Products.GetById(Guid.Parse(id)),CancellationToken.None);
        Assert.IsType<Api.Models.Product>(result.product);
    }
    [Fact]
    public async Task GetByIdProductInvalid_Return_Null()
    {
        var result = await _handler.Handle(new Requests.Products.GetById(Guid.NewGuid()),CancellationToken.None);
