...
namespace BugStore.Test.Handlers.Order.Query.GetById;
...
public class HandlerGetByIdOrder
{
    ...
    public async Task GetByIdOrderOk_Return_Order()
    {
        ...
        var result = await _handler.Handle(new Api.Requests.Orders.GetById(id),CancellationToken.None);
        Assert.IsType<Api.Models.Order>(result.Order);
    } 
    [Fact]
